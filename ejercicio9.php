...
<body>
    ...
    <main>
      ...
      if(isset($_POST["aceptar"]) && $_FILES["imagen"]["error"] == 0) {
        ...
        $direccion = "imagenes/";
        $imageFileType = strtolower(pathinfo($direccion, PATHINFO_EXTENSION));

        $check = getimagesize($_FILES["imagen"]["tmp_name"]);
        if($check !== false) {
            echo '<div class="imagen">';
            echo '<img width=600px height=600px src="imagenes/' . basename($_FILES["imagen"]["name"]) . '" alt="' . $nombreImagen . '">';
            echo '<p>' . $nombreImagen . '</p>';
            echo '</div>';
        } else {
